...
function draw(){
  ...
  if(openSet.length > 0){
    ...
    for(var i = 0; i < openSet.length; i++){
      if(openSet[i].f < openSet[winner].f){ //.g for dikstra, .f for A*
        winner = i; //
      }
    }
...
function makeNewGrid(){
  ...
  for(var i = 0; i < grid.length; i++){
    for(var j = 0; j < grid[i].length; j++){
      switch(txt[i][j]){
        case "." :
          grid[i][j] = new Spot(i,j); break;
        case "#" :
          grid[i][j] = new blockSpot(i,j); break;
        case "A":
          grid[i][j] = new startSpot(i,j); start = grid[i][j]; break;
        case "B":
          grid[i][j] = new stopSpot(i,j);  end = grid[i][j]; break;
        case "m":
          grid[i][j] = new mountainSpot(i,j); break;
        case "f":
          grid[i][j] = new forestSpot(i,j); break;
        case "w":
          grid[i][j] = new waterSpot(i,j); break;
        case "g":
          grid[i][j] = new grassSpot(i,j); break;
        case "r":
          grid[i][j] = new roadSpot(i,j); break;
      }
    }
  }
