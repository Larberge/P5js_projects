...
//the map API
var curl = "https://api.mapbox.com/styles/v1/mapbox/dark-v9/static/0,0,"+zoomLvl+","+globalCenterX+","+globalCenterY+
    ...
    if(timeMillis > limA && timeMillis < limB){
      while(true){
        if(eqs[dateNum][3] == date){
          ...
          fill(0,255,100, 200);
          stroke(0,255,100,200);
          ellipse(eqs[dateNum][0],eqs[dateNum][1],eqs[dateNum][2],eqs[dateNum][2]);
          dateNum -= 1;
        }
        else{
          date = eqs[dateNum][3];
